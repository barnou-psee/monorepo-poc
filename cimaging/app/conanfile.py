...
class CImagingApp(ConanFile):
    ...
    def requirements(self):
        ...
        self.requires("metavision-foo/0.0.1")
        self.requires("metavision-bar/0.0.1")
        self.test_requires("gtest/1.14.0")

    def set_version(self):
        content = load(self, os.path.join(self.recipe_folder, "CMakeLists.txt"))
        self.version = re.search(r"project\(cimaging-app VERSION (.*) LANGUAGES CXX\)", content).group(1)
    ...
    def package_info(self):
        self.cpp_info.set_property("cmake_file_name", "CImaging")
        self.cpp_info.components["bar"].libs = ["cimaging_app"]
        self.cpp_info.components["bar"].includedirs = ["cpp/include"]
        self.cpp_info.components["bar"].set_property("cmake_target_name", "CImaging::app")
